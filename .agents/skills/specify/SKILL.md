...
3. **Create the spec feature directory**:
   ...
   Specs live under the default `specs/` directory unless the user explicitly provides `FEATURE_DIR`.

   **Resolution order for `FEATURE_DIR`**:
   1. If the user explicitly provided `FEATURE_DIR` (e.g., via environment variable, argument, or configuration), use it as-is
   2. Otherwise, auto-generate it under `specs/`:
      - Check `.sp/init-options.json` for `feature_numbering` (preferred) or `branch_numbering` (deprecated, migration only — will be removed in a future release)
   ...
   **Create the directory and spec file**:
   - `mkdir -p FEATURE_DIR`
   - Resolve the active `spec-template` through the Constitutions preset/template resolution stack (equivalent to `specify preset resolve spec-template`)
   - Copy the resolved `spec-template` file to `FEATURE_DIR/spec.md` as the starting point
   - Set `SPEC_FILE` to `FEATURE_DIR/spec.md`
   - Persist the resolved path to `.sp/feature.json`:
     ```json
     {
