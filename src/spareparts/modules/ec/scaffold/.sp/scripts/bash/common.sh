...
# Find repository root by searching upward for .sp directory
# This is the primary marker for constitutions projects
find_constitutions_root() {
    local dir="${1:-$(pwd)}"
...
# *contains* .sp/), for non-interactive / CI use — e.g. running a Constitutions
...
resolve_constitutions_init_dir() {
    ...
    if [[ ! -d "$init_root/.sp" ]]; then
        echo "ERROR: PLAYBOOKS_INIT_DIR is not a Constitutions project (no .sp/ directory): $init_root" >&2
        return 1
    fi
    printf '%s\n' "$init_root"
}

# Get repository root, prioritizing .sp directory
# This prevents using a parent repository when constitutions is initialized in a subdirectory
get_repo_root() {
    # Explicit project override wins (see resolve_constitutions_init_dir).
    if [[ -n "${PLAYBOOKS_INIT_DIR:-}" ]]; then
        resolve_constitutions_init_dir
        return
    fi

    # First, look for .sp directory (constitutions's own marker)
    local constitutions_root
    if constitutions_root=$(find_constitutions_root); then
        echo "$constitutions_root"
        return
    fi
...
# the git extension) or implicitly via .sp/feature.json.
get_current_branch() {
...
# Safely read .sp/feature.json's "feature_directory" value.
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.sp/feature.json"
...
# Persist a feature_directory value to .sp/feature.json.
...
_persist_feature_json() {
    ...
    # Ensure .sp/ directory exists
    mkdir -p "$repo_root/.sp"
...
get_feature_paths() {
    ...
    # --no-persist so pure path resolution never writes .sp/feature.json,
    ...
    #   2. .sp/feature.json "feature_directory" key (persisted by specify command)
    ...
    if [[ -n "${PLAYBOOKS_FEATURE_DIRECTORY:-}" ]]; then
    ...
    elif [[ -f "$repo_root/.sp/feature.json" ]]; then
        local _fd
        ...
        if [[ -n "$_fd" ]]; then
            ...
            echo "ERROR: Feature directory not found. Set PLAYBOOKS_FEATURE_DIRECTORY or ensure .sp/feature.json contains feature_directory." >&2
            return 1
        fi
    else
        echo "ERROR: Feature directory not found. Set PLAYBOOKS_FEATURE_DIRECTORY or run the specify command to create .sp/feature.json." >&2
        return 1
    fi
...
get_invoke_separator() {
    ...
    local integration_json="$repo_root/.sp/integration.json"
    local separator="."
    local parsed=0
...
format_constitution_command() {
    # Constitutions commands install without a namespace (e.g. /specify, /plan),
...
#   1. .sp/templates/overrides/
#   2. .sp/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .sp/extensions/<ext-id>/templates/
#   4. .sp/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.sp/templates"
    ...
    local presets_dir="$repo_root/.sp/presets"
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.sp/extensions"
    if [ -d "$ext_dir" ]; then
